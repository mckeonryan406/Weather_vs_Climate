...
#stationID = "USC00097600"  # Rome GA
#stationID = "USC00046719" # Pasadena CA
#stationID = "USC00194313"  # Lowell MA
#stationID = "USC00439984"  # Woodstock VT
#stationID = "USC00438556"  # Thetford VT

# Pick a year for temperature
currentYear = 2017  # MAKE DYNAMIC IN SHINY


# ***********************************  Filter Stations and Plot **************************
# load all GHCN Weather Stations
...
hFactor = setNames(data.frame(matrix(ncol = 3, nrow = lengthOfRecord)), c("year", "hNum", "cNum"))
...
for (i in startYear:endYear) {
  ...
  hFactor[rowNum,2]  = max(yearMax$rank, na.rm = TRUE) # find the crossover where rank exceeds temperature --  the H factor
  ...
  hFactor[rowNum,3]  = max(yearMin$rank, na.rm = TRUE)
}

xLow = min(hFactor$year)
xHigh = max(hFactor$year)

#plot(hFactor$year,hFactor$cNum)
...
day_summary = setNames(data.frame(matrix(ncol = 11, nrow = 366)), c("day", "Tmin", "Tmax", "Tmean","Tmin2SD","Tmax2SD","meanPrecip","cumSumPrecip","precipSD","TmaxRecord","TminRecord"))
...
for (i in 1:366) {
  dayDF = norm30[norm30$julDay == i, ]  # subset the data to a single julian day
  day_summary[i,1] = i
  ...
  dayDF = TempPreALL[TempPreALL$julDay == i, ]      # reset the dayDF to the whole data set to calculate the record hi and low
...
### Map of Weather Stations from the Global Historical Climatology Network - The blue pin indicates the location of the current station being plotted below. The darkness of the point indicates the length of the record for each station.  Click on points to get the station ID number.
...
### Monthly Temperture and Precipition Normal (1981-2010)  

```{r, fig.cap="Classic Climograph showing average monthly temperature (gray bars) and precipitation (blue line). "}
...
```{r, fig.cap="Blue to Red colors reflect recorded daily high temperature, gray and white represent missing data."}
...
### Daily Temperature vs. Long-term Averages
```{r,  fig.width=9, fig.height=5, fig.cap="This shows daily temperature ranges over the course of a year vs. the average high and low (light gray) and the record high and low since 1980 (dark gray)."}
...
ggplot() +
  ...
  geom_linerange(data = day_summary, mapping = aes(x = day, ymin = TminRecord, ymax = TmaxRecord), color = "#949494", size = 1.5) +  # daily record high and low temperature range 
  geom_linerange(data = day_summary, mapping = aes(x = day, ymin = Tmin, ymax = Tmax), color = "#b0b0b0", size = 1.5) +        # 30 yr mean hi/lo
  geom_linerange(data = currentYearDF, mapping = aes(x = julDay, ymin = degF.y, ymax = degF.x, color = Tmean), alpha = 0.6, size = 1.5) +   # observed data for particular year
...
```{r, fig.cap="The Hotness Index is defined as the number of days H that the daily maximum Fahrenheit temperature is H or above.  The points show the annual variation, the red line smooths the data using a loess filter and the gray shading is the 95% CI."}
hFactorPlot = ggplot() +
...
### Monthly Mean Temperature Anomaly Comparison
```{r}

# anomaly plots comparing past with present at a monthly scale

# anomaly for each month is a 10 year average that is subtracted from the 30 normal (1981-2010) monthly average for the same month
...
latest10DF = TempPreALL[TempPreALL$year.x > 2009 & TempPreALL$year.x < 2020, ]
...
for (i in 1:12) {
  ...
  norm30MonthDF[i,"PrecipMean"] = sum(currentMonthDF$precipInches, na.rm = TRUE)/10  # divide by the length (in years) to get the average monthly precip.
  ...
  oldMonthDF[i,"PrecipMean"] = sum(currentMonthDF$precipInches, na.rm = TRUE)/10  # divide by the length (in years) to get the average monthly precip.
  ...
  latestMonthDF[i,"TempAnom"] = latestMonthDF[i,"TempMean"] - norm30MonthDF[i,"TempMean"]
  latestMonthDF[i,"PrecipAnom"] = latestMonthDF[i,"PrecipMean"] - norm30MonthDF[i,"PrecipMean"]
